...
class WhoisUtil
{
    ...
    public function __construct()
    {
        ...
        require_once PROJECT_ROOT . 'data/whois/src/whois.main.php';
        require_once PROJECT_ROOT . 'data/whois/src/whois.utils.php';
     ...
     * @return string|array
     */
    public function get($domain, $returnHtml = true)
    {
        try {
            $domains      = explode('.', $domain);
            $domainTld    = array_pop($domains);
            $configServer = $this->whois_arr[$domainTld] ?? '';
            if (!empty($configServer)) {
                $this->whois->UseServer($domainTld, $configServer);
            }
            ...
            $html          = array();
            $html['basic'] = $this->showWhois($result);
            $server        = $this->whois->Query['server'] ?? '';
            if (!empty($result)) {
                $html['native_data'] = $this->utils->showHTML($result);
            }
            return $html;
        } catch (Exception $e) {
            return '';
     ...
     */
    private function showWhois($result)
    {
        $rawData = $result['rawdata'] ?? [];
        // 错误信息
        if (!empty($result['query']['status']) && $result['query']['status'] == 'error') {
            $err = '';
            if (!empty($result['query']['error'])) {
                if (is_array($result['query']['error'])) {
                    $err .= implode('<br>', $result['query']['error']);
                } elseif (is_string($result['query']['error'])) {
                    $err .= $result['query']['error'];
                }
            }
            if (!empty($result['query']['errstr'])) {
                if (is_array($result['query']['errstr'])) {
                    $err .= implode('<br>', $result['query']['errstr']);
                } elseif (is_string($result['query']['errstr'])) {
                    $err .= $result['query']['errstr'];
                }
            }
            return ['error' => ($err ?: 'err.request')];
        }
        $endStartWith = '>>>';
        $tableRow     = [];
        $fetchHeader  = [
        ...
        if (!empty($result['regrinfo']['domain']['nserver'])) {
            foreach ($result['regrinfo']['domain']['nserver'] as $domain => $ip) {
                $nameServers[] = [
                    'name'  => $serverKey,
                    'value' => sprintf('%s (%s)', $domain, $ip),
                ];
            }
        } else {
            $fetchHeader[] = $serverKey;
        }
        foreach ($rawData as $item) {
            foreach ($fetchHeader as $header) {
                if (strpos($item, $header) !== false) {
                    list(, $val) = explode($header, $item);
                    // $val        = preg_replace(
                    //     '@(https?://([-\w\.]+)+(:\d+)?(/([\w/_\.]*(\?\S+)?)?)?)@',
            ...
            if (strpos($item, $endStartWith) === 0) {
                break;
            }
        }
        $tableRow = array_merge($tableRow, $nameServers);
        return Render::list2Table($tableRow);
    }
    ...
    public static function get1stDomain($string)
    {
        ...
        try {
            ...
            return $domain->getName() . '.' . $domain->getSuffix();
        } catch (Exception $e) {
            $str    = explode('.', $string);
            $length = count($str);
