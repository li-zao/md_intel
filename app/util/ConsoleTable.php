...
class ConsoleTable
{
    ...
    public function addRow(array $data = null)
    {
        ...
        if (is_array($data)) {
            foreach ($data as $col => $content) {
                $this->data[$this->rowIndex][$col] = $this->filterContent($content);
            }
    ...
    /**
     * Filter the content to prevent display incorrectly
    ...
    private function getVisualWidth($index, $content)
    {
        ...
        $chineseLength = $this->getChineseLength($cleanContent);
        $len = $colWidth + $delta - $this->countEmojis($cleanContent);
        return $chineseLength ? $len - $chineseLength : $len + $chineseLength;
    }
        ...
        foreach ($this->data as $row) {
            if (is_array($row)) {
                foreach ($row as $x => $cell) {
                    ...
                    $textLength += $this->getChineseLength($content);
     ...
     * @return float
     */
    private function getChineseLength($str)
    {
        $pattern = '/[\x{4e00}-\x{9fa5}]/u';
        $matches = [];
        preg_match_all($pattern, $str, $matches);
        return intval(count($matches[0]) * self::CHINESE_RATIO);
    }
