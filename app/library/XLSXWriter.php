...
class XLSXWriter
{
    ...
    public function __construct($data = [])
    {
        ...
        $this->addCellStyle(self::FORMAT_TEXT, null);
        if (!empty($data['col_width'])) {
            $this->col_width = $data['col_width'];
        }
        if (!empty($data['merge_cells'])) {
            $this->merge_cells = $data['merge_cells'];
        }
    }
    ...
    protected function initializeSheet($sheet_name, $col_widths = array(), $auto_filter = false, $freeze_rows = false, $freeze_columns = false)
    {
        ...
        if (empty($col_widths) && !empty($this->col_width)) {
            $col_widths = $this->col_width;
        }
        $merge_cells = [];
        if (!empty($this->merge_cells)) {
            $merge_cells = $this->merge_cells;
        }
    ...
    public function writeSheetHeader($sheet_name, array $header_types, $col_options = null)
    {
        ...
        if (!$suppress_row) {
            ...
            foreach ($header_row as $c => $v) {
                $cell_style_idx = empty($style) ? $sheet->columns[$c]['default_cell_style'] : $this->addCellStyle(self::FORMAT_TEXT, json_encode(isset($style[0]) ? $style[$c] : $style));
    ...
    public function writeSheetRow($sheet_name, array $row, $row_options = null, $col_widths = [])
    {
        if (empty($sheet_name)) {
            return;
        }

        $this->initializeSheet($sheet_name, $col_widths);
        $sheet = &$this->sheets[$sheet_name];
        if (count($sheet->columns) < count($row)) {
            $default_column_types = $this->initializeColumnTypes(array_fill($from = 0, $until = count($row), self::FORMAT_TEXT));//will map to n_auto
        ...
        if (!empty($row_options)) {
            ...
            $hidden    = isset($row_options['hidden']) && $row_options['hidden'];
            $collapsed = isset($row_options['collapsed']) && $row_options['collapsed'];
    ...
    public static function convert_date_time($date_input) //thanks to Excel::Writer::XLSX::Worksheet.pm (perl)
    {
        ...
        if (preg_match("/(\d+):(\d{2}):(\d{2})/", $date_time, $matches)) {
            ...
            $seconds = intval($hour * 3600 + $min * 60 + $sec) / (24 * 60 * 60);
        }
