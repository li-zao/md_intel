...
class XdbSearcher
{
    public const HEADER_INFO_LENGTH = 256;
    public const VECTOR_INDEX_ROWS = 256;
    public const VECTOR_INDEX_COLS = 256;
    public const VECTOR_INDEX_SIZE = 8;
    public const SEGMENT_INDEX_SIZE = 14;
    ...
    public function search($ip)
    {
        ...
        if ($this->vectorIndex != null) {
            ...
            $sPtr = self::getLong($this->contentBuff, self::HEADER_INFO_LENGTH + $idx);
            $ePtr = self::getLong($this->contentBuff, self::HEADER_INFO_LENGTH + $idx + 4);
        } else {
        ...
        $dataPtr = null;
        $l       = 0;
        $h       = ($ePtr - $sPtr) / self::SEGMENT_INDEX_SIZE;
        while ($l <= $h) {
            $m = ($l + $h) >> 1;
            $p = $sPtr + $m * self::SEGMENT_INDEX_SIZE;

            // read the segment index
            $buff = $this->read($p, self::SEGMENT_INDEX_SIZE);
            if ($buff == null) {
    ...
    public static function loadHeader($handle)
    {
        ...
        $buff = fread($handle, self::HEADER_INFO_LENGTH);
        if ($buff === false) {
            return null;
        }

        // read bytes length checking
        if (strlen($buff) != self::HEADER_INFO_LENGTH) {
            return null;
        }
    ...
    {
        if (fseek($handle, self::HEADER_INFO_LENGTH) == -1) {
            return null;
        }

        $rLen = self::VECTOR_INDEX_ROWS * self::VECTOR_INDEX_COLS * self::SEGMENT_INDEX_SIZE;
